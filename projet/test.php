   ...
   public function generer() {
      ...
      if(isset($_POST['qcm_id']) AND $_POST['qcm_id']==$id) {
         ...
         foreach($this->questions as $i=>$question) {
            echo 'Question '.$i.' : '.$question->getQuestion().'<br /><br />';
            if($_POST['qcm_q'.$i]==$question->getNumBonneReponse()) {
               echo 'Bonne réponse!<br /><br />';
               $nbrBonneReponses++;
            }
            else
               echo 'Mauvaise réponse : '.$question->getReponse($_POST['qcm_q'.$i])->getText().'<br />';
            echo 'La bonne réponse était : '.$question->getBonneReponse()->getText().'<br /><br />';
            echo 'Explication : '.$question->getExplication();
            echo '<hr />';
         }
         $note = round($nbrBonneReponses/count($this->questions)*20);
         echo 'Note : '.$note.'/20<br />';
         if(isset($this->appreciation[$note]))
            echo 'Appréciation : '.$this->appreciation[$note];
      }
      else {
         echo '<form method="post" />';
         echo '<input type="hidden" value="'.$id.'" name="qcm_id" />';
         foreach($this->questions as $i=>$question) {
            echo '<fieldset>Question '.($i+1).' : '.$question->getQuestion().'<br />';
            foreach($question->getReponses() as $j=>$reponse) {
               echo '<input type="radio" name="qcm_q"'.$i.'value='.$j.' />';
               echo $reponse->getText().'<br />';
            }
            echo '</fieldset><br />';
         }
