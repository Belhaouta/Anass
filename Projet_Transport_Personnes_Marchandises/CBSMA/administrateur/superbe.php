...
<body>
<h1><u><i>Calendrier Ordinaire</i></u></h1>
...
while( $resultat = $reqx->fetch()){
    ...
    $start->modify('-1 days'); // 'now' n'est pas nécéssaire, c'est la valeur par défaut
    $end= new DateTime("now+7 day");      // 'on ajoute 30 jours à la date courante pour un affichage qui couvrira depuis aujourd'huy jusqu'à 30 jours plus tard..
    ...
    foreach (new DatePeriod($start,new DateInterval('P1D')/*pas de 1 jour*/ , $end)as $dt){
        ...
        if($i!=0 && $j!=0){
            ...
            $bdd=connecter();
            $tab=array();
            $req = $bdd->prepare('SELECT count(*)  FROM course WHERE Date_course ="'.$jour.'"  and id_heure="'.$horaire.'" and Status="simple"');
            $req->execute();
            $resultatx = $req->fetch();
            $req->closeCursor();
            if($resultatx[0] == 0) {    $tab[0]=false;  }
            else{
                $req = $bdd->prepare('SELECT * FROM course WHERE Date_course ="'.$jour.'"  and id_heure="'.$horaire.'" and Status="simple"');
                $req->execute();
